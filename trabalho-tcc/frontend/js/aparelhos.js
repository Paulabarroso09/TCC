document.addEventListener('DOMContentLoaded', () => {
    console.log("Tela de aparelhos carregada com sucesso!");
  
    // Caso queira futuramente adicionar dinâmica (ex: listar do banco):
    // fetch('/api/aparelhos').then(...)
...
function mostrarDetalhes(aparelhoId) {
    const aparelho = aparelhosData[aparelhoId];
    const detalhesContainer = document.getElementById('detalhes-aparelho');
    
    // Atualizar informações do aparelho
    document.getElementById('detalhes-nome').textContent = aparelho.nome;
...
function salvarManutencao(event) {
    ...
    // Obter aparelho atual (simulação - em app real você teria isso salvo)
    const aparelhoAtual = 'principal'; // Isso seria dinâmico na aplicação real
    
    // Salvar no localStorage
    salvarNoHistorico(aparelhoAtual, dados);
    
    // Recarregar histórico
    carregarHistorico(aparelhoAtual);
